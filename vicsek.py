...
def mean_theta(x_in, y_in, theta_in, *params):
    ...
    for i in range(N):
        ...
        mean_theta[i] = np.arctan(np.mean(np.cos(theta_in[nearest_neighbour]))/np.mean(np.sin(theta_in[nearest_neighbour])))
    return mean_theta
...
def vel(i):
    param = param_list[i]
    data = compute(*param)
    np.save(f'./data/viscek_n={param[6]:0,.1f}N={int(param[0])}.npy', data)
    Vx = np.cos(data[2, :, :])
    Vy = np.sin(data[2, :, :])
